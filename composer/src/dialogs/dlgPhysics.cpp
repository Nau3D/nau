...
DlgPhysics::setupGrid() {
	...
	PropertyManager::createOrderedGrid(m_PGMat, PhysicsMaterial::GetAttribs(), order);
	m_PGMat->SetSplitterLeft(true, true);
	m_PGGlobal->Clear();
	m_PGGlobal->AddPage(wxT("properties"));
	PropertyManager::createGrid(m_PGGlobal, PhysicsManager::GetAttribs());
}
...
DlgPhysics::update() {
	...
	if (m_Active != "") {
		...
		PropertyManager::updateGrid(m_PGMat, PhysicsMaterial::GetAttribs(), (AttributeValues *)&mat);
		wxPGProperty *p = m_PGMat->GetPropertyByName("SCENE_TYPE");
		p->Enable(false);
		
		m_PGMat->Refresh();

		PropertyManager::updateGrid(m_PGGlobal, PhysicsManager::GetAttribs(), (AttributeValues *)PhysicsManager::GetInstance());
	}
}
...
DlgPhysics::OnMaterialPropsChange( wxPropertyGridEvent& e) {
	...
	PropertyManager::updateProp(m_PGMat, topProp, PhysicsMaterial::GetAttribs(), (AttributeValues *)&mat);
