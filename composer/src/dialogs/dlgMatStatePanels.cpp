...
void DlgMatStatePanels::setPanel(wxSizer *siz, wxWindow *parent){
	...
	PropertyManager::createOrderedGrid(m_PG, nau::material::IState::GetAttribs(), order);
...
void DlgMatStatePanels::OnProcessPanelChange(wxPropertyGridEvent& e){
	...
	PropertyManager::updateProp(m_PG, name.ToStdString(), nau::material::IState::GetAttribs(), (AttributeValues *)m_glState);
}


void DlgMatStatePanels::updatePanel(){

	PropertyManager::updateGrid(m_PG, nau::material::IState::GetAttribs(), (AttributeValues *)m_glState);
	m_PG->Refresh();
}
