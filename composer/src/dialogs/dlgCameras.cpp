...
DlgCameras::setupPanel(wxSizer *siz, wxWindow *parent) {
	...
	sizH3->Add(m_BAdd, 0, wxALL | wxGROW | wxHORIZONTAL , 5);
...
DlgCameras::setupGrid() {
	...
	PropertyManager::createOrderedGrid(m_PG, Camera::GetAttribs(), order);
...
void DlgCameras::update() {
	...
	PropertyManager::updateGrid(m_PG, Camera::GetAttribs(), (AttributeValues *)cam.get());
...
void DlgCameras::OnPropsChange( wxPropertyGridEvent& e) {
	...
	if (prop == "Viewport") {
		...
		PropertyManager::updateProp(m_PG, topProp, Camera::GetAttribs(), (AttributeValues *)cam.get());
