...
void DlgMaterials::setupTexturesPanel(wxSizer *siz, wxWindow *parent) {
	...
	PropertyManager::createOrderedGrid(pgTextureProps, ITextureSampler::GetAttribs(),order);
...
void DlgMaterials::setTextureUnit(int index){
	...
	pgTextureProps->SetPropertyValue(wxT("Texture Type"),wxString(ITexture::GetAttribs().getListStringOp(ITexture::DIMENSION, texture->getPrope(ITexture::DIMENSION)).c_str()));
	...
	PropertyManager::updateGrid(pgTextureProps, ITextureSampler::GetAttribs(), (AttributeValues *)ts);
...
void DlgMaterials::OnProcessTexturePropsChange( wxPropertyGridEvent& e) {
	...
	if (name == wxT("Texture Unit")) {
		...
		PropertyManager::updateProp(pgTextureProps, name.ToStdString(), ITextureSampler::GetAttribs(), (AttributeValues *)ts);
}
...
DlgMaterials::setupColorPanel(wxSizer *siz, wxWindow *parent) {
	...
	PropertyManager::createOrderedGrid(pgMaterial, ColorMaterial::GetAttribs(), order);
...
DlgMaterials::OnProcessColorChange( wxPropertyGridEvent& e){
	...
	PropertyManager::updateProp(pgMaterial, name.ToStdString(), ColorMaterial::GetAttribs(), (AttributeValues *)cm);
}
...
DlgMaterials::updateColors(std::shared_ptr<Material> &mm) {
	if (mm)
		PropertyManager::updateGrid(pgMaterial, ColorMaterial::GetAttribs(), (AttributeValues *)mm.get());
}
...
DlgMaterials::setupShaderPanel(wxSizer *siz, wxWindow *parent) {
	...
	sizerS->Add(sizH1,1,wxALL|wxALIGN_CENTER,5);
	...
	sizerS->Add(sizerS_SB,0,wxALL|wxALIGN_CENTER,5);
	...
	sizerU->Add(pgShaderUniforms,1, wxALL | wxGROW | wxEXPAND ,5);

	siz->Add(sizerS,0, wxALL | wxALIGN_CENTER,5);
	siz->Add(sizerU,1, wxALL | wxEXPAND ,5);
...
void DlgMaterials::addUniform(wxPGProperty *pdefaultBlock, ProgramValue  &u, int showGlobal) {
	...
	pid2 = pgShaderUniforms->AppendIn(pid, new wxStringProperty(wxT("Data Type"), wxPG_LABEL, wxString(Enums::GetDataTypeToString()[u.getValueType()].c_str())));
