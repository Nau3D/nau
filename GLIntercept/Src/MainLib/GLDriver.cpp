...
bool GLDriver::ReConfigure()
{
  ...
  //Recreate the extension function handler
  extensionFunction->removeOverrides();
...
//##################### MY ALTER
bool GLDriver::Reset(){
	printf("DEBUG: resetting config\n");
  //Read in the configuration data
  configData = defaultConfigData;
  //configData.ReadConfigData(); //No config file data
...
bool GLDriver::AddOverrideFunction(const string & funcName, void * overrideFunctionPtr, void *origFuncPtr)
{
  ...
  if(!extensionFunction)
  {
    ...
    LOGERR(("GLDriver::AddOverrideFunction")); 
