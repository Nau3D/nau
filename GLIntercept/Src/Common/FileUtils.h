...
public:
  ...
  enum FileProperties
  {
    GLIFP_NORMAL   = 1 << 0L,     // Standard read/write access  
    GLIFP_READONLY = 1 << 1L,     // File is flagged as read only
  };
  ...
  //                      (cannot set incompatable flags such as GLIFP_NORMAL and GLIFP_READONLY)
  //
  //  Returns:
  //    True is returned on success, false if otherwise.
  //
