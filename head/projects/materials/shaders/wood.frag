...
void main() {
	...
	float blend = sin(worldPos.x*4);
	float t = turbulence(vec3(worldPos) * scale);

	//blend = fract(frequency * worldPos.z * scale);
	blend = fract(frequency * worldPos.z * scale + noiseScale * turbulence(vec3(worldPos) * scale));
	...
	blend = pow(blend, frequency/2) + t*0.5;
