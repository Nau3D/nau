...
void
GLArrayOfTextures::prepare(unsigned int bufferBindingPoint) {

	if (m_IntProps[BUFFER_ID] != 0 && m_Buffer != NULL) {
...
GLArrayOfTextures::clearTextures() {
	...
	for (unsigned int i = 0; i < m_UIntProps[TEXTURE_COUNT]; ++i) {
		RESOURCEMANAGER->getTextureByID(m_TextureIDs[i])->clear();
	}
}
...
GLArrayOfTextures::clearTexturesLevel(int l) {
	...
	for (unsigned int i = 0; i < m_UIntProps[TEXTURE_COUNT]; ++i) {
		RESOURCEMANAGER->getTextureByID(m_TextureIDs[i])->clearLevel(l);
	}
}
...
GLArrayOfTextures::generateMipmaps() {
	...
	for (unsigned int i = 0; i < m_UIntProps[TEXTURE_COUNT]; ++i) {
		RESOURCEMANAGER->getTextureByID(m_TextureIDs[i])->generateMipmaps();
	}
}