...
__device__ void sampleUnitHemisphereCosWeighted(float3& normal, float3& seed, float3& point)
{
	float2 r;
	float3 U,V;
...
__device__ float4 sampleAreaLight(float3 surfaceNormal, float3 hitPoint, 
				 float3 lightPos, float3 lightNormal, float lightSizeX,  float lightSizeY)  
{
	...
	float2 r;
	float dot1 = dot(hitPoint-lightPos, lightNormal);
	...
	for (unsigned int i = 0; i < lightSamples; ++i) 
	{
		...
		//seedj = tea<16>(b,a+1);
		random2D(hitPoint, r);
		float3 lPos = lightPos + make_float3(1,0,0) * lightSizeX * r.x + make_float3(0,0,1) * lightSizeY * r.y;
		float3 lDir = lPos - hitPoint;
		float lightDist = length(lDir);
		lDir = normalize(lDir);

		float NdotL = max(0.0f, dot(surfaceNormal, lDir));
		if (NdotL) {
			dot1 = max(0.0f,dot(lightNormal, -lDir));

			shadow_prd.result = make_float4(1.0f);
 			optix::Ray shadow_ray( hitPoint, lDir, Shadow, 0.000002, lightDist+0.01 );
