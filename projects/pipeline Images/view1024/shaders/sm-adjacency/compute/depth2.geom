...
void shrinkTriangle3(in vec4 p[6], in vec3 n[6], float pixelDiagonal) {
	...
	vec3 pos[3]; //pos2, pos3 e pos4 são para testes futuros
	vec3 lightDir = normalize (vec3(V * -lightDirection));
...
/* 	normalss = normalize(cross(vec3(edge[0].xy, 0), vec3(edge[1].xy,0)));
	...
	//Triangulos Adjacentes
	bool t1Luz=false, t2Luz=false, t3Luz=false; //Indicadores se os triangulos adjacentes existem/estão em luz
	
	vec3 edgeT1, edgeT2, edgeT3;
	edgeT1 = p[1].xyz - p[0].xyz;
//	edgeT1[1] = p[2].xyz - p[1].xyz;
	edgeT2 = p[3].xyz - p[2].xyz;
//	edgeT2[1] = p[4].xyz - p[3].xyz;
	edgeT3 = p[5].xyz - p[4].xyz;
//	edgeT3[1] = p[0].xyz - p[5].xyz;
	
  	if ( length(edgeT1) < 0.001 ){ //p[1] não existe
		t1Luz = true; 
	}
	else { //p[1] existe
		normalss = normalize( cross(vec3(edgeT1),vec3(edge[0])) );
		if (dot(normalss, lightDir) > 0) //Ver se ele está em luz
			t1Luz=true;
	}
	
	if ( length(edgeT2) < 0.001 ){ //p[3] não existe
		t2Luz = true; 
	}
	else { //p[3] exisre
		normalss = normalize( cross(vec3(edgeT2),vec3(edge[1])) );
		if (dot(normalss, lightDir) > 0) //Ver se ele está em luz
			t2Luz = true;
	}
	
	if ( length(edgeT3) < 0.001 ){ //p[5] não existe
		t3Luz = true; 
	} 
	else { //p[5] existe
		normalss = normalize( cross(vec3(edgeT3),vec3(edge[2])) );
		if (dot(normalss, lightDir) > 0) //Ver se ele está em luz
			t3Luz = true;
	}
...
/*    	if (length(newEdge4.xy) < pixelDiagonal*k || 
				  length(newEdge5.xy) < pixelDiagonal*k || 
				  length(newEdge6.xy) < pixelDiagonal*k)
		return; */
...
/* 	if (emLuz > 1) { //Emitir 1 triangulo (Pos)
		gl_Position = vec4(pos[0], 1);
		EmitVertex();
		...
		if (emLuz == 1) { //Emitir 2 Triangulos (Pos e Pos2)
			gl_Position = vec4(pos[0], 1);
			EmitVertex();
			gl_Position = vec4(pos[1], 1);
			EmitVertex();
			...
			EmitVertex();
			gl_Position = vec4(pos2[1], 1);
			EmitVertex();
			gl_Position = vec4(pos2[2], 1);
			EmitVertex();
			EndPrimitive();
		}
		else { //Emitir 4 Triangulos (Pos, Pos2, Pos3 e Pos4)
			gl_Position = vec4(pos[0], 1);
			EmitVertex();
			gl_Position = vec4(pos[1], 1);
			EmitVertex();
			...
			EmitVertex();
			gl_Position = vec4(pos3[1], 1);
			EmitVertex();
			gl_Position = vec4(pos3[2], 1);
			EmitVertex();
			EndPrimitive();
			
			gl_Position = vec4(pos4[0], 1);
			EmitVertex();
			gl_Position = vec4(pos4[1], 1);
			EmitVertex();
			gl_Position = vec4(pos4[2], 1);
			EmitVertex();
			EndPrimitive();
		}
	} */
...
/* 	//Triangulo central
	if (DataIn[0].index == 3 && DataIn[2].index == 0 && DataIn[4].index == 2) {
		//return;
	}
	//Triangulo de Baixo
	...
	if (DataIn[0].index == 3 && DataIn[2].index == 2 && DataIn[4].index == 4) {
		//return;
	}
	//Triangulo da direita
	if (DataIn[0].index == 3 && DataIn[2].index == 5 && DataIn[4].index == 0) {
		//return;
	} */
