...
// Não corrigiu pixeis incorrectos		
...
void encolher(in vec4 screenPos[6], in float pixelDiagonal){

	vec3 pedge[3]; vec3 pos[3];

	vec2 side0N = normalize(screenPos[2].xy - screenPos[0].xy);
	vec2 side1N = normalize(screenPos[4].xy - screenPos[2].xy);
	vec2 side2N = normalize(screenPos[0].xy - screenPos[4].xy);

	float kk = 2.0;
	if (length(side0N.xy) < pixelDiagonal*kk || 
		length(side1N.xy) < pixelDiagonal*kk || 
		length(side2N.xy) < pixelDiagonal*kk)
		    return;
		  
	vec3 normal = normalize(cross(vec3(side0N.xy, 0), vec3(side1N.xy,0)));
	if (normal.z > 0)
		  return;	
	
	pos[0] = screenPos[0].xyz;
	pos[1] = screenPos[2].xyz;
	pos[2] = screenPos[4].xyz;
	
	pos[0].xy = screenPos[0].xy - normalize(-side0N+side2N)*pixelDiagonal;
	pos[1].xy = screenPos[2].xy - normalize(side0N-side1N)*pixelDiagonal;
	pos[2].xy = screenPos[4].xy - normalize(side1N-side2N)*pixelDiagonal;

	
	// testar inversões
	pedge[0] = vec3(side0N,0);
	pedge[1] = vec3(side1N,0);
	pedge[2] = vec3(side2N,0);
	
	if (inversion(pedge, pos, screenPos))
		  return;
	
	//Teste para ver o tamanho novo das edges
	vec3 newEdge0 = pos[1] - pos[0];
	vec3 newEdge1 = pos[2] - pos[0];
	vec3 newEdge2 = pos[2] - pos[1];
	...
	vec3 nnormal = cross(vec3(side0N.xy, 0), vec3(side1N.xy,0));
	if (normal.z >= -0.0)
		return;
	
	// Emitir vertices
	gl_Layer = 0;
	DataOut.pos = vec4(pos[0].xyz, screenPos[0].w);
	gl_Position = vec4(pos[0].xyz, screenPos[0].w);
	DataOut.bBoxMin = vec2(0);
	DataOut.bBoxMax = vec2(0);
	EmitVertex();

	DataOut.pos = vec4(pos[1].xyz, screenPos[2].w);
	gl_Position = vec4(pos[1].xyz, screenPos[2].w);
	DataOut.bBoxMin = vec2(0);
	DataOut.bBoxMax = vec2(0);
	EmitVertex();

	DataOut.pos = vec4(pos[2].xyz, screenPos[4].w);
	gl_Position = vec4(pos[2].xyz, screenPos[4].w);
	DataOut.bBoxMin = vec2(0);
	DataOut.bBoxMax = vec2(0);
	EmitVertex();
...
void expandir(in vec4 screenPos[6], in float pixelDiagonal, in vec2 bMin, in vec2 bMax) {
	vec3 pedge[3]; vec3 pos[3];
...
/* 	vec3 nnormal = cross(vec3(newEdge0.xy, 0), vec3(newEdge1.xy, 0));
	if (nnormal.z >= -0.0)
		return; */
