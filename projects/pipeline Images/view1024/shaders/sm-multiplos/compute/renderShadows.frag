...
void main()
{
	...
	float dispY = 0.5/textureSize(shadowMaps,0).y; //displacement for Y
  	...
  	if (intensity > 0.0) {
		...
		//float d0 =  min( c0, min( c1, min( c2, min( c3, c4 ) ) ) ); //isto não é bem no centro do pixel.
		...
		//float d1 = max( s0, max( s1, max( s2, max( s3, s4 ) ) ) ); //isto não é bem no centro do pixel.
		float d1 = min( s0, min( s1, min( s2, min( s3, s4 ) ) ) ); //isto não é bem no centro do pixel.
		//float d1 = texture(shadowMaps, vec4(p.xy,0,p.z)); //Shadow Map Expandido
