...
static __forceinline__ __device__ void cosine_sample_hemisphere(const float u1, const float u2, float3& p)
{
  ...
  p.y = sqrtf( fmaxf( 0.0f, 1.0f - r*r ) );
}

static __forceinline__ __device__ void uniform_sample_hemisphere(const float u1, const float u2, float3& p)
{
  const float cosalpha   = sqrtf( 1 - u1 * u1 );
  const float beta = 2.0f*M_PIf * u2;
  p.z = cosalpha * cosf( beta );
  p.x = cosalpha * sinf( beta );
  p.y = u1;
}
