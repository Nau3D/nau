...
AttributeValues::isValidi4(Int4Property prop, ivec4 &value) {
	...
	if (attr->getRangeDefined()) {
		ivec4 &max = *(std::dynamic_pointer_cast<ivec4>(attr->getMax()));
		ivec4 &min = *(std::dynamic_pointer_cast<ivec4>(attr->getMin()));
