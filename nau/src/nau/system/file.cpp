...
File::GetFilesInFolder(std::string path, std::string extension, std::vector<std::string> *files) {
	...
	if ((dir = opendir(path.c_str())) != NULL) {
		while ((ent = readdir(dir)) != NULL) {
			if (ent->d_type == /*S_IFREG*/DT_REG && GetExtension(ent->d_name) == extension) {
...
void 
nau::system::File::FixSlashes(std::string & name) {

// Although windows supports forward slashes full file or dir names will
// always hava a backslash
...
File::GetPath(const std::string &fn) {
	...
	std::string s;
	if (found == fn.npos) 
		s = fn.substr(0,fn.size()-1); // clone string
	else
		s= fn.substr(0,found);
...
File::CleanFullPath(const std::string &fn) {
	...
	std::string aux, left, right;
	size_t pos, posL;
	right = s;
	pos = right.find_first_of("\\/");
	while (pos != s.npos && pos < s.size() - 2) {

		right = s.substr(pos + 1);
		if (right[0] == '.' && right[1] == '.' && pos > 0) {

			left = s.substr(0, pos);
			posL = left.find_last_of("\\/");
			s = left.substr(0, posL) + right.substr(2, right.size() - 2);
		}
		else
			posL = pos + 1;
		pos = s.find_first_of("\\/", posL);
	}
	return s;
...
File::getRelativeTo (File &file) {
	...
	relative.m_Filename = file.m_Filename;
	return relative.getFullPath() + relative.m_Filename;
}
